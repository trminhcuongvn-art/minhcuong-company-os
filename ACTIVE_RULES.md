...
## ANTI-PROCRASTINATION SYSTEM v2 — Henry chốt 2026-06-05 16:40
4 lớp bắt buộc, áp dụng ngay:

### L1 — NO-WAIT RULE
Gặp blocker bất kỳ → làm task khác có output ngay, KHÔNG chờ. Tối đa 2 tool calls tìm context, sau đó tiếp tục hoặc chuyển task.

### L2 — EVIDENCE-GATE (brief)
Mọi hourly brief BẮT BUỘC kèm ít nhất 1 evidence cụ thể: commit hash / file path / test output / log line.
Brief không có evidence = INVALID, không được gửi cho Henry.

### L3 — 30-MIN OUTPUT CHECK (cron)
Mỗi 30 phút tự hỏi: "Trong 30 phút qua có file/commit/output mới không?"
Nếu KHÔNG → dừng planning/logging, làm 1 task có output ngay trước khi làm gì khác.

### L4 — TASK FORMAT BẮT BUỘC
Mỗi task trước khi bắt đầu phải định nghĩa:
- INPUT: đã có gì
- OUTPUT: file/commit/test cụ thể
- DEADLINE: +N phút
- FALLBACK: nếu blocked sau 10 phút làm gì thay thế
Không có đủ 4 trường → không được bắt đầu task.
